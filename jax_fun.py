...
def make_psi_ratio(state0, state1, psi):
    """
...
def Eloc(state, t, U, psi, Lsite, g):
    ...
    #print('make_psi_all_hopped:')
    #print(make_psi(all_hopped, psi))
    #print('make_psi_state:')
    #print(make_psi(state, psi))
    Psi = make_psi(all_hopped, psi) / make_psi(state, psi)
    #print('Psi')
    #print(Psi)
...
def jump_nearest(state, Lsite, psi):
    ...
    #print('states_hopped:')
    #print(states_hopped)
    all_psi = make_psi(states_hopped, psi) 
...
def Metropolis_single(psi, state_init, Lsite, t, U, g, nthermal, npoints, nacc):
    ...
    for istep in range(nstep):
        ...
        ratio_psi = make_psi(state_proposal, psi) / make_psi(state, psi)
...
def Metropolis(psi, states_init, Lsite, t, U, g, nthermal, npoints, nacc):
    ...
    for istep in range(nstep):
        ...
        psi_ratio = make_psi_ratio_vmapped(states, states_proposal, psi) 
...
def grad_g(psi, states_init, Lsite, t, U, g, nthermal, npoints, nacc):
    ...
    = D(x) g^{D(x)-2}, where D(x) is the number of double occupation of | x >
    The full estimator is given by the mean value of D(x) * g^{D(x)-2} * ( E(x) -E.mean )
    """
    batch, Ne = states_init.shape
    states = jnp.array(list(states_init))
    ...
    grad_logpsi = jnp.zeros((npoints, batch))
    num_accept = 0
    ...
    for istep in range(nstep):
        ...
        if istep in range(nthermal, nstep+1, nacc):
            ...
            #
            grad_logpsi_auto = logpsi_square_grad(states, psi, Lsite, g)
            grad_logpsi = grad_logpsi.at[ipoints,:].set(grad_logpsi_auto)
            #
            ipoints += 1

    grad = jnp.multiply( double_occ, jnp.power(g, -1) )
    #print('grad_all:')
    #print(grad_all)
    E_mean = Elocs.mean()
    grad_g = jnp.multiply( grad, Elocs - E_mean )
    grad_mean = grad_g.mean() 
    grad_auto = jnp.multiply( grad_logpsi, Elocs - E_mean )
    grad_auto_mean = grad_auto.mean()
 
    return grad_mean*2, grad_auto_mean, E_mean



def optimize_g(psi, states_init, Lsite, t, U, nthermal, npoints, nacc, opt_nstep):

    import optax

    optimizer = optax.adam(learning_rate = 1e-2*5)
